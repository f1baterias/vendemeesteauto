...
function toggleZoom(event) {
    ...
    if (zoomActivo) {
        // Posicionar zoom inicial centrado en el click
...
function togglePesos() {
    const toggle = document.getElementById('toggle-pesos');
    const input = document.getElementById('cotizacion-dolar');

    mostrarEnPesos = toggle.checked;

    if (mostrarEnPesos) {
        input.disabled = false;
        input.focus();
        // Si ya tiene un valor, aplicar
...
function actualizarIndicadorCotizacion() {
    ...
    if (indicador) {
        if (mostrarEnPesos && cotizacionDolar > 0) {
            indicador.textContent = `1 USD = $ ${cotizacionDolar.toLocaleString('es-AR')} ARS`;
...
function inicializarEventos() {
    ...
    // Toggle pesos y cotización
    document.getElementById('toggle-pesos').addEventListener('change', togglePesos);
    document.getElementById('cotizacion-dolar').addEventListener('input', actualizarCotizacion);
